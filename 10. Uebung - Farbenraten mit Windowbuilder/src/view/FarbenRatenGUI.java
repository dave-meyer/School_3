...
public class FarbenRatenGUI {
	...
	private JFrame rateFrame;			//Hauptframe für sämtliche GUI-Komponenten
	...
	/**
	 * Standardeinstellung und GUI aufbauen
	 */
	public void los() {
		size = normal;
		initialize();
	...
	private void initialize() {
		...
		rateFrame = new JFrame();
		rateFrame.setTitle("Farbenratespiel");
		rateFrame.setBounds(100, 100, 550, 550); //Frame 100,100 Pixel v. linken, oberen BS-Rand 450x550
		rateFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		rateFrame.getContentPane().setLayout(new BorderLayout(10, 10)); //Zwischen Bereichen ein 10,10 Abstand
		
		//Panel für Farbbuttons instanziieren (CENTER)
		panel = new JPanel();
		panel.setBorder(new TitledBorder("JPanel - GridLayout(size,size) - CENTER d. JFrames"));
		rateFrame.getContentPane().add(panel, BorderLayout.CENTER);
		panel.setLayout(new GridLayout(size, size));	//size x size = Anzahl Farbbuttons
		
		//Textfield für Statistikwerte instanziieren (SOUTH)
		text = new JTextField();
		text.setBorder(new TitledBorder("JTextField - SOUTH d. JFrames"));
		rateFrame.getContentPane().add(text, BorderLayout.SOUTH);
		...
		colorPanel.add(text);
		rateFrame.getContentPane().add(colorPanel, BorderLayout.NORTH);
		...
		levelPanel.add(b2);
		levelPanel.add(b3);
		rateFrame.getContentPane().add(levelPanel, BorderLayout.EAST);
	...
	{	
		...
		rateFrame.repaint();	//alles neu zeichnen
